...
def stream_videos(stream, fps=None, subtype=None, res=None, progressive=None):
    return stream.streams.filter(fps=fps, subtype=subtype, res=res, type="video", progressive=progressive).order_by(
        "resolution").desc()


def youtube_url(res_num, stream, save_path="./output"):
    videos = stream_videos(stream, subtype="mp4", res=res_num)
    temp = "./temp"
    for video in videos:
    ...
    if videos[index].is_progressive:
        ...
        convert_progressive(video=f"{temp}/{title}.mp4", audio=f"{temp}/{audio_title}.mp4", title=title,
                            save_path=save_path)
